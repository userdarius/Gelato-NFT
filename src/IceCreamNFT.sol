...
contract CircleNFT is ERC721Enumerable, GelatoVRFConsumer {
    ...
    // Mapping from token ID to its radius
    mapping(uint256 => uint256) private _radiusData;

    constructor(GelatoVRFInbox _inbox, address _dedicatedMsgSender) ERC721("CircleNFT", "CNFT") {
        inbox = _inbox;
    ...
    function getRarity(uint256 tokenId) public view returns (string memory) {
        uint256 radius = _radiusData[tokenId];
        return determineRarity(radius);
    }

    function determineRarity(uint256 radius) private pure returns (string memory) {
        if (radius <= 20) {
            return "Ultra Rare";
        } else if (radius <= 40) {
            return "Rare";
    ...
    function randomRadius(uint256 randomness) private pure returns (uint256) {
        return (uint256(keccak256(abi.encodePacked(randomness))) % 50) + 10; // A random value between 10 and 60
    }
    ...
    function randomColor(uint256 randomness) private pure returns (string memory) {
        ...
        uint256 index = uint256(keccak256(abi.encodePacked(randomness))) % 10;
        return colors[index];
    }

    function createSVG(uint256 radius, string memory color) private pure returns (string memory) {
        return string(
            abi.encodePacked(
